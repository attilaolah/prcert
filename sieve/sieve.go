...
func Sieve() chan int {
	...
	go func() {
		// In order to generate the nth prime we only need multiples of primes
		// ≤ sqrt(nth prime).  Thus, the merging goroutine will receive from
		// this channel much slower than this goroutine will send to it,
		// causing the buffer to accumulate and block this goroutine from
		// sending to `primes`, causing a deadlock.  The solution is to use a
		// proxy goroutine to do automatic buffering.
